...
async function deleteProject(req, res) {
  ...
  const deleted = await projectService.deleteUserProject(userId, projectId);
  if (!deleted) {
    res.status(404).json({ error: "Project not found" });
    return;
  }

  res.status(200).json({ success: true, message: "Project deleted successfully", project: deleted });
}
