/**
 * Project API client - talks to Sentimind API gateway
 *
 * NOTE: Safe refactor - logic kept identical to previous src/lib/projectApi.ts
 */

const GATEWAY_BASE = (import.meta.env.VITE_API_BASE as string) || "http://localhost:8000";
...
async function request<T>(path: string, options: RequestInit & { accessToken: string }): Promise<T> {
  ...
  const res = await fetch(`${GATEWAY_BASE}/projects${path}`, { ...init, headers });
