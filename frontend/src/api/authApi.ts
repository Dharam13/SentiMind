 ...
 *
 * NOTE: Safe refactor - logic kept identical to previous src/lib/authApi.ts
 */

const AUTH_BASE = (import.meta.env.VITE_API_BASE as string) || "http://localhost:8000";
...
async function request<T>(path: string, options: RequestInit & { accessToken?: string } = {}): Promise<T> {
  ...
  if (!res.ok) {
    const msg = (data as any)?.error || (data as any)?.details?.[0]?.message || "Request failed";
    throw new Error(msg);
  }
  return data as T;
}
